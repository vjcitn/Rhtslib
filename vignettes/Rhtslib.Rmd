...
in `r Biocpkg("Rhtslib")`, you have two options:

### Option 1: Using Rhtslib::pkgconfig() (traditional method)
...
### Option 2: Using standard pkg-config (recommended)

`r Biocpkg("Rhtslib")` now provides a standard pkg-config file that can be
used with the `pkg-config` utility. This is the recommended method for new
packages as it follows the standard pkg-config protocol.

First, ensure that `pkg-config` can find the Rhtslib pkg-config file by
setting the `PKG_CONFIG_PATH` environment variable. This can be done in your
`src/Makevars` file:
...
This method is compatible with standard build systems and tools that use
pkg-config, and works on Linux, macOS, and Windows (when using Rtools).
